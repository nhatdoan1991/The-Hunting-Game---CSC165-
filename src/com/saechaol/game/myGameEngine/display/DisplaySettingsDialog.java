package com.saechaol.game.myGameEngine.display;
...
/**
 * This class provides a dialog box for specifying display settings. It receives
 * a GraphicsDevice object, queries the device to obtain the supported display
 * modes, and provides a GUI which allows the user to select from the available
 * options. The class remembers the user-selected values and provides methods
 * for obtaining the selected values, including DisplayMode (width, height,
 * bit-depth, and refresh rate), and whether or not to use FSEM (Full Screen
 * Exclusive Mode).
 * <P>
 * After creating a DisplaySettingsDialog, call showIt() to make the dialog
 * visible. The visible dialog is modal (meaning the invoking program cannot
 * continue until the dialog is dismissed). Hitting either "OK" or "Cancel" on
 * the dialog dismisses (hides) the dialog, but does not delete it; the invoking
 * code can make it visible again by calling showIt() again.
 * <P>
 * Note that this class does <i>not</i> actually alter any display settings or
 * modes; it simply gives the invoking code a convenient graphical method for
 * allowing the user to indicate the desired settings. The invoking code is
 * responsible for fetching the user's selections from the dialog and using that
 * information to actually alter any desired display settings.
 *
...
public class DisplaySettingsDialog extends JDialog implements ActionListener {
	...
	/**
	 * Creates a DisplaySettingsDialog for the specified GraphicsDevice and allows
	 * the user to choose desired display mode values. Call showIt() in the created
	 * dialog to make it visible. The dialog is modal (must be dismissed once shown
	 * before the invoking program can continue). The created dialog retains its
	 * settings even when hidden; accessors are provided for the invoking code to
	 * determine the display values selected by the user.
	 */
	...
	private Vector<String> getDisplayModeList(DisplayMode[] modes) {
		...
		for (DisplayMode m : modes) {
			...
			String s = "" + width + "x" + height + ", " + depth + "-bit color, " + rate + "-Hz refresh rate";
	...
	/**
	 * Makes the dialog visible and waits for the user to dismiss it by hitting
	 * either "OK" or "Cancel". Hitting "OK" causes the values shown in the dialog
	 * GUI to be saved in the dialog. Note however that the dialog does not actually
	 * install any values into the underlying machine; the invoking application is
	 * responsible for reading the dialog values and installing them if desired.
	 */
	public void showIt() {
		...
		currentResolutionLabel.setText(
				"Current Resolution:  " + width + "x" + height + ", " + depth + "-bits, " + refreshRate + "-Hz ");
	...
	/**
	 * Returns a copy of the currently selected DisplayMode, or null if no display
	...
	public DisplayMode getSelectedDisplayMode() {
		...
		if (selectedDisplayMode != null) {
			copy = new DisplayMode(selectedDisplayMode.getWidth(), selectedDisplayMode.getHeight(),
	...
	/**
	 * Returns a boolean indicating whether or not the user has specified that Full
	 * Screen Exclusive Mode (FSEM) should be used.
	 */
	 ...
	 * with the specified DisplayMode. If a match is found, returns the index in the
	 * ComboBox of the matching item; otherwise, returns zero.
	 */
	...
	/**
	 * Returns true if the display mode defined by the specified String matches the
	 * specified DisplayMode object values; false otherwise.
	 */
	 ...
	 * rate of the mode defined by the string, and returns a DisplayMode object
	...
	private DisplayMode toDisplayMode(String modeString) {
		...
		DisplayMode mode = new DisplayMode(Integer.valueOf(widthString), Integer.valueOf(heightString),
				Integer.valueOf(depthString), Integer.valueOf(refreshRateString));
		return mode;
	}

	/**
	 * Respond to Action Events from Dialog buttons (OK, Cancel). An "OK" event
	 * causes the current GUI values to be saved in the dialog; "Cancel" results in
	 * no changes to the dialog. In either event the dialog is hidden afterwards.
	 */
